...
export default function BottomRightCorner() {
  return (
    <motion.a
      ...
      className="absolute bottom-0 right-0 p-3 pt-5 pl-8 sm:p-4 sm:pt-6 sm:pl-10 md:p-6 md:pt-8 md:pl-14 bg-[#f0f0f0] rounded-tl-[1.5rem] sm:rounded-tl-[2rem] md:rounded-tl-[3.5rem] flex items-center gap-3 sm:gap-4 md:gap-6 cursor-pointer hover:opacity-80 transition-opacity"
    >
      ...
      {/* Circle Icon */}
      <div className="bg-[rgba(30,50,90,0.05)] w-10 h-10 md:w-14 md:h-14 rounded-full flex items-center justify-center border border-[rgba(30,50,90,0.1)] shrink-0">
        <ArrowUpRight className="text-[rgba(30,50,90,0.8)] w-5 h-5 md:w-6 md:h-6" />
      </div>

      {/* Info column */}
      <div className="flex flex-col gap-1">
        <span className="text-[16px] md:text-[20px] font-normal text-[rgba(30,50,90,0.95)]">下载简历</span>
        <div className="flex items-center gap-1 text-[rgba(30,50,90,0.6)] cursor-pointer hover:text-[rgba(30,50,90,0.8)] transition-colors">
          <span className="text-[12px] md:text-[15px] font-normal">产品策划 / 海外市场</span>
